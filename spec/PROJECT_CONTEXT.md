...
Step 7 (Supabase persistence) is complete. The reviewer + senior flows, tags, examples, points, multiplier-bonus schedule, app settings, branding favicon, and per-user theme are all DB-backed under RLS. 24 migrations applied to the work-account Supabase project.
...
- **8.7** — Quarantine via `Image.Hidden`. The `reviews_update_quarantine` trigger (migration 6) has always maintained `photos.is_quarantined`; what 8.7 adds is the SmugMug-side side effect — toggling the image's `Hidden` flag so it stops appearing in public album views and search while still living in its camp_week album with all URLs/metadata intact. The very first cut tried physically relocating images into a global Unlisted "Photo Reviewer — Quarantined" album via `Album!collectimages` (then `AlbumImage` PATCH against `Uris.ImageAlbum`); both produced subtle copy-rather-than-move semantics that wasted hours debugging SmugMug's many-to-many image/album graph for a problem that already had a one-line answer in the form of `Image.Hidden`. The current implementation is a single PATCH `/api/v2/image/<imageKey>` with `{ Hidden: true }` for quarantine and `{ Hidden: false }` for release; senior delete on a quarantined photo is a noop on the SmugMug side (the image stays Hidden=true until manually deleted). Migration 23 added `'quarantine_move'` to the `sync_kind` enum and a now-unused `smugmug_config.quarantine_album_key` column; migration 24 drops the column (the enum value stays — Postgres can't remove enum values without a type rewrite, and the audit category is still meaningful as "any quarantine-driven SmugMug visibility change"). Two server-only files: [lib/smugmug/quarantine.ts](../lib/smugmug/quarantine.ts) is now a one-function module exporting `setImageHidden(imageKey, hidden)`; [lib/smugmug/sync/quarantine.ts](../lib/smugmug/sync/quarantine.ts) hosts `runQuarantineReconcile`, which reads `(is_quarantined, current_status)`, picks an action per the rule [`is_quarantined=true` → Hidden=true; `is_quarantined=false` + `status=deleted` → noop; `is_quarantined=false` + `status<>deleted` → Hidden=false], calls the helper, and writes a `quarantine_move` sync_log row in either success or failed terminal state. The route handler at [app/api/smugmug/quarantine/route.ts](../app/api/smugmug/quarantine/route.ts) takes `{ photoId }`, runs the reconcile under the service-role client, and always returns 200 — drift surfaces as a `quarantine_move` / `failed` row on the existing Admin → SmugMug → Sync log card (the AdminSmugMug `KindPill` got a `Quarantine` label, and `lib/sync-log.ts → SyncKind` got the new union member). Client wiring is fire-and-forget through [lib/quarantine-trigger.ts](../lib/quarantine-trigger.ts), called from `ReviewScreen.commitDecision` after a flag-with-quarantine submission and from `FlagReview.resolve` after senior accept/delete on a previously-quarantined photo. The reviewer never blocks on the SmugMug round-trip; the route writes the audit trail. Idempotency is implicit: PATCHing `Hidden` to its current value is a harmless no-op on SmugMug, so the reconcile doesn't bother with a pre-check.
...
app/
  ...
  api/smugmug/
    ...
    quarantine/route.ts   # step 8.7 quarantine endpoint (POST, any-authenticated). Body { photoId }. Delegates to lib/smugmug/sync/quarantine.ts → runQuarantineReconcile under the service-role client; reads photos.is_quarantined + current_status to pick action (quarantine | release | noop) and PATCHes Image.Hidden on SmugMug accordingly, then writes a quarantine_move sync_log row. Always returns 200 — drift lands in sync_log for the admin to see.
components/
...
lib/
  ...
  smugmug/                # server-only SmugMug v2 API client (step 8.1)
    ...
    quarantine.ts         # step 8.7 SmugMug helper: setImageHidden(imageKey, hidden) → PATCH /api/v2/image/<imageKey> with JSON body { Hidden: true|false }. That single call is the entire SmugMug-side mechanism — quarantine flips Hidden=true (image stops appearing in public album views and search but stays in its album with all URLs intact), release flips Hidden=false. SmugMug 301-redirects bare image keys to versioned form; the smugmugFetch wrapper transparently re-signs and follows.
    sync/                 # iD Tech-specific tree interpretation (step 8.3 — translates SmugMug's generic Node tree into our Division/Location/Year/Week schema)
      ...
      quarantine.ts       # step 8.7 quarantine reconcile core — runQuarantineReconcile reads the photo, picks an action from (is_quarantined, current_status) per the rule [is_quarantined=true → Hidden=true; is_quarantined=false + status=deleted → noop; is_quarantined=false + status<>deleted → Hidden=false], calls setImageHidden, and writes a quarantine_move sync_log row in success or failed terminal state. Never throws to the caller — the route handler always returns 200 and drift lands in sync_log.
  supabase/
...
supabase/
  migrations/             # 24 SQL migrations applied to the work-account project (see SCHEMA_SPEC.md for the table)
  tests/
...
8.7 — Quarantine via Image.Hidden
The reviews_update_quarantine trigger (migration 6) already maintains photos.is_quarantined. The SmugMug-side side effect — hiding the image from public album views and search when quarantined, restoring it when a senior accepts — is application-side, triggered by observing the column flip. A Route Handler at app/api/smugmug/quarantine is called from the client right after a flag-with-quarantine submission and again after a senior accept/delete on Flag Review; it PATCHes /api/v2/image/<imageKey> with { Hidden: true|false }. Idempotent for free (PATCHing Hidden to its current value is a harmless no-op on SmugMug). On failure, the DB and SmugMug can drift — surfaced as a quarantine_move / failed row in sync_log rather than blocking the user. The original 8.7 design relocated images into a global Unlisted "Photo Reviewer — Quarantined" album, but Image.Hidden is the simpler tool: no album to find-or-create, no AlbumImage relationship to relocate, no URL refresh after the fact.
8.8 — Tests + seed cleanup
A new e2e SQL test, supabase/tests/e2e_smugmug_sync_flow.sql, exercises six scenarios that mirror the SQL the real sync engine emits — a SQL test can't reproduce SmugMug HTTP calls, but it can validate the contract those calls have with the database: (1) clean-slate insert, (2) re-run is a no-op (no UPDATE when nothing drifted, smugmug_image_id unique constraint blocks duplicate insert), (3) orphans missing from SmugMug get deleted if unreviewed and preserved if reviewed, (4) re-parenting moves the row's camp_week_id rather than inserting a duplicate, (5) reviewer-queue ordering follows priority desc, captured_at <queueOrder> and excludes non-pending rows, (6) mode-switch "clear the queue" deletes only unreviewed pending rows (terminal-status and reviewed-pending stay).
Migration 25 (drop_placeholder_dev_data) deletes the placeholder photos, camp week, and location seeded by migration 13 — they were dev scaffolding that shouldn't ship alongside real SmugMug data. The four placeholder *divisions* stay because the 8.3 folder sync rewrites their smugmug_folder_id in place once it discovers the real ids; deleting them would either re-create on next sync or strip the bootstrap rows the admin needs to flip `synced=true` on. Filters use the unique `placeholder-` prefix so reconciled rows (whose ids were rewritten to real SmugMug values) stay put. Reviews on placeholder photos cascade-delete via the reviews.photo_id FK.
The other e2e tests (e2e_review_flow, e2e_flag_review_flow, e2e_reviewer_stats) used to hard-code placeholder smugmug_image_id values; they now insert their own division/location/week/photo fixtures up front (under the service role) before flipping to the authenticated role for the review inserts. Fixture rows roll back at the end of the transaction so nothing leaks between runs.
Deferred to V2
