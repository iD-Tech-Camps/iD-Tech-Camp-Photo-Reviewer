...
**Active phase: step 8 — SmugMug API integration.** Substeps 8.1, 8.2, 8.3, 8.4, 8.5, and 8.6 are done:
...
- **Quarantine folder move is still trigger-only.** `photos.is_quarantined` flips on flag-with-quarantine, but no SmugMug folder move runs yet. That's 8.7.
...
| 8 | **SmugMug API integration** — admin-curated import pool with folder priority (not full auto-ingest); 8.1–8.6 done, 8.7–8.8 pending | 🟡 Active |
