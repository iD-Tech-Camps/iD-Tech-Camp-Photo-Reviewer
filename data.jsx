...
// Reasons for rejecting a photo — shown as selectable chips in reject dialog
...
// Reasons for flagging for admin review
const FLAG_REASONS = [
...
// Generate a pleasant placeholder photo via CSS gradient + stripes + label.
// Indexed by id so photos stay stable across renders.
const PHOTO_PALETTES = [
  ['oklch(0.72 0.12 55)', 'oklch(0.55 0.08 30)'],   // sunset
  ['oklch(0.75 0.10 150)','oklch(0.45 0.08 160)'],  // forest
  ['oklch(0.72 0.10 220)','oklch(0.48 0.10 230)'],  // lake
  ['oklch(0.82 0.08 85)', 'oklch(0.55 0.06 65)'],   // sand
  ['oklch(0.70 0.10 15)', 'oklch(0.40 0.10 25)'],   // rose
  ['oklch(0.78 0.09 120)','oklch(0.50 0.09 140)'],  // moss
  ['oklch(0.76 0.09 250)','oklch(0.50 0.08 255)'],  // twilight
  ['oklch(0.80 0.09 45)', 'oklch(0.52 0.09 35)'],   // amber
  ['oklch(0.73 0.10 190)','oklch(0.45 0.08 200)'],  // teal
  ['oklch(0.78 0.08 75)', 'oklch(0.50 0.06 70)'],   // straw
];

function photoPaletteFor(id) {
  const n = id ? id.split('').reduce((a, c) => a + c.charCodeAt(0), 0) : 0;
  return PHOTO_PALETTES[n % PHOTO_PALETTES.length];
}

// A placeholder "photo" that looks photographic-ish: gradient + soft vignette
// + abstract shapes, so the UI reads like a real image viewer without us
// drawing actual kids. Labeled with the camp and activity.
function PhotoPlaceholder({ photo, compact = false, hideLabel = false }) {
  ...
  return (
    <div style={{
      ...
      {/* Abstract scene suggestion — blobs behind a horizon */}
      <div style={{
...
// Teams = internal iD Tech departments (reviewers are staff, not counselors on site)
const LEADERBOARD = [
...
// Example library curated by admin
const EXAMPLES = {
  good: [
...
Object.assign(window, {
  PHOTO_TAGS, REJECT_REASONS, FLAG_REASONS, SESSION_PHOTOS, PhotoPlaceholder, LEADERBOARD, TEAMS,
  BADGES, RECENT_ACTIVITY, EXAMPLES, ADMIN_USERS, photoPaletteFor,
});
