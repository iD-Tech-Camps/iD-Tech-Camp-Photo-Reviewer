// Review session — focused on friction-free action
// Three decisions → each opens a lightweight modal:
//   Approve → 5-star rating (+ optional positive tags)
//   Reject  → reason chips (become tags on the photo)
//   Flag    → reason chips + custom message for the admin
// Points surface as a bottom-right toast — never blocks the photo.

function ReviewScreen({ onComplete, onExit, showExamplesDrawer, setShowExamplesDrawer, toast }) {
  const [index, setIndex] = React.useState(0);
  const [decisions, setDecisions] = React.useState({});
  // modal state: null | 'approve' | 'reject' | 'flag'
...
// Big action button
// ———————————————————————————————————————————————————————————
function BigAction({ color, tone, onClick, icon, label, shortcut }) {
...
// ———————————————————————————————————————————————————————————
// Modal shell — centered card with backdrop
// ———————————————————————————————————————————————————————————
...
function ModalHeader({ eyebrow, title, tone }) {
...
// ———————————————————————————————————————————————————————————
// Approve modal — 5-star rating + optional positive tags
// ———————————————————————————————————————————————————————————
const POSITIVE_TAGS = PHOTO_TAGS.filter(t => t.color !== 'rose');

function ApproveModal({ photo, onCancel, onConfirm }) {
...
// ———————————————————————————————————————————————————————————
function RejectModal({ photo, onCancel, onConfirm }) {
...
// ———————————————————————————————————————————————————————————
// Flag modal — reason chips + custom message
// ———————————————————————————————————————————————————————————
function FlagModal({ photo, onCancel, onConfirm }) {
  const [reasons, setReasons] = React.useState([]);
...
// Batch complete
// ———————————————————————————————————————————————————————————
function SessionComplete({ decisions, onHome, onAnother }) {
...
Object.assign(window, { ReviewScreen, SessionComplete });
